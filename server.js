...
app.prepare()
    .then(() => {
        ...
        server.get('/s/:id', (req, res) => {
            const actualPage = '/startup'
            const queryParams = { id: req.params.id }
            app.render(req, res, actualPage, queryParams)
        })

        server.get('/api/s/all', (req, res) => {
            return res.status(200).json(highlights);
        })

        server.get('/api/s/:_id', (req, res) => {

            for (var i = 0; i < highlights.length; i++) {
                for (var j = 0; j < highlights[i].startups.length; j++) {
                    if (highlights[i].startups[j]._id === req.params._id)
                        return res.status(200).json(highlights[i].startups[j]);
                }
            }
